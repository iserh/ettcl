...
class XCRepoDataset(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        if resp.headers["Content-Type"].startswith("text/html"):
            ...
            dataset_url = "https://docs.google.com/uc?export=download&id=%s&confirm=%s" % (
        ...
        trn_split = datasets.SplitGenerator(name=datasets.Split.TRAIN, gen_kwargs=dict(fpath=trn_fpath))
        tst_split = datasets.SplitGenerator(name=datasets.Split.TEST, gen_kwargs=dict(fpath=tst_fpath))
    ...
    def _generate_examples(self, fpath):
        ...
        with open(fpath, "r", encoding="latin-1") as f:
            ...
            for i, example in enumerate(map(json.loads, f)):
                ...
                yield i, {"title": example["title"], "text": example["content"], "labels": example["target_ind"]}
